...
ggplot(gsa.nondirectional.matrix.m, aes(Gene, GO.Biological.Process)) + 
...
summary.table <- data.frame(gene=gene.names, pval=sprintf("%.2e", hr.random.effects.pval), hr.text=sprintf("%.2f [%.2f, %.2f]", hr.random.effects.estimate, hr.random.effects.lower, hr.random.effects.upper))
...
forest(hr.estimates, hr.variances, atransf=exp, psize=1, efac=0, slab=names(hr.rma.objs), xlab="Hazard Ratio",xlim=c(-2,3.5), at=log(c(0.25,0.5,1,2,4)), ilab = sprintf("%.2e", hr.random.effects.pval), ilab.xpos = 1.5)
op <- par(cex=1, font=2)
...
#forest(estimates, variances, atransf=exp, psize=1, efac=0, slab=names(rma.objs), xlab="Hazard Ratio",xlim=c(-2,2), at=log(c(0.25,0.5,1,2,4)))
forest(d.index.estimates, d.index.variances, atransf=exp, psize=1, efac=0, slab=names(d.index.rma.objs), xlab="D-Index",xlim=c(-2,3.5), at=log(c(0.25,0.5,1,2,4)), ilab = sprintf("%.2e", d.index.random.effects.pval), ilab.xpos = 1.5)
op <- par(cex=1, font=2)
text(-2, length(gene.names) + 1.5, "Gene", pos=4)
text(1.5, length(gene.names) + 1.5, "p-value")

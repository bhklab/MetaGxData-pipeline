...
meta.correlation.vals <- apply(correlation.matrix, 1, function(gene.correlation.vals) get.meta.correlation(pcc = gene.correlation.vals, n=sapply(esets.with.gene, ncol)))

lapply(esets, function(eset) {
  if(!(entrez.id %in%fData(eset)$EntrezGene.ID)) {
    return(NA)
  }
  entrez.id.index <- which(fData(eset)$EntrezGene.ID == entrez.id)
  
  entrez.id.exprs.vals <- exprs(eset)[entrez.id.index,]
  
  # Subset to the top 1000 genes by variance
  variance.vals <- apply(exprs(eset), 1, var)
  genes.to.keep <- variance.vals >= sort(variance.vals, decreasing = TRUE)[1000]
  eset <- eset[genes.to.keep,]
  
  cor.vals <- apply(exprs(eset), 1, function(gene.exp.vals) cor(gene.exp.vals, entrez.id.exprs.vals, method='pearson'))
  
  names(cor.vals) <- sub("geneid.", "", names(cor.vals))
  
  c5.bp.geneset <- loadGSC("c5.bp.v5.1.entrez.gmt")
  
  out <- runGSA(cor.vals, geneSetStat = "gsea", gsc=c5.bp.geneset, ncpus = 4)

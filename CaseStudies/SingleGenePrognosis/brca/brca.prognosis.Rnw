...
lapply(esets, function(eset) {
  ...
  cor.vals <- apply(exprs(eset), 1, function(gene.exp.vals) cor(gene.exp.vals, exprs(eset)[entrez.id.index,], method='pearson'))
  
  names(cor.vals) <- sub("geneid.", "", names(cor.vals))
  
  c5.bp.geneset <- loadGSC("c5.bp.v5.1.entrez.gmt")
  
  out <- runGSA(cor.vals, geneSetStat = "gsea", gsc=c5.bp.geneset)

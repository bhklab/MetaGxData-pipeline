...
random.effects.pval[{{current.index}}] <- rma.random$pval
# create.forest.plot returns log2(HR)
random.effects.estimate[{{current.index}}] <- exp(as.vector(rma.random$b))
random.effects.lower[{{current.index}}] <- exp(rma.random$ci.lb)
random.effects.upper[{{current.index}}] <- exp(rma.random$ci.ub)
@

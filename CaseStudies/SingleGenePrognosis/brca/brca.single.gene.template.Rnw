...
<<singlegene_metaanalysis_forestplot_{{current.index}}, fig.width=5, fig.height=7, out.width="0.5\\textwidth", cache=TRUE>>=
xlim=c(-3,3)
alim=c(-3,3)
...
forest(hr.rma.random, xlab="Hazard Ratio", atransf=exp, refline = 0, annotate=FALSE, addfit=FALSE, ylim=c(-2.5,length(hr.stat.vals) + 3), at=log(c(0.25,0.5,1,2,4)), xlim=xlim, alim=alim, main=paste0("Meta-analysis: ", gene.names[{{current.index}}]))
abline(h=0, lwd=1)
...
o.text <- paste(sprintf("HR: %.3f [%.3f,%.3f]\n", exp(hr.rma.random$b), exp(hr.rma.random$ci.lb), exp(hr.rma.random$ci.ub)), sprintf("p = %.1E", hr.rma.random$pval))

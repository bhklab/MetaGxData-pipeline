## create esets according to patientselection.config specifications


## -----------------------------------------------------------------------------
## functions from Levi's createEsetList.R (ovarian)
## -----------------------------------------------------------------------------
...
#   remove.samples <- NULL
#   load("./esets/duplicate_list.rda")
#   duplicates <- duplicate_list
  
  # for(n in 1:length(duplicates)){
  #   datasets <- gsub(paste(delim, ".+", sep=""), "", duplicates[[n]])
  #   ## find which dataset has largest
  #   if(remove.duplicates == "keep.smallest"){
  #   remove <- which.min(lapply(datasets, function(x){length(sampleNames(get(x)))}))
  #   } else {
  #     remove <- which.max(lapply(datasets, function(x){length(sampleNames(get(x)))}))
  #   }
  #   remove.samples <- c(remove.samples, duplicates[[n]][-remove])
  #   rm(remove)
  # }
#   ## remove.samples is now a list of the replicates to remove
...
#!!! keep duplicates in largest or smallest dataset
